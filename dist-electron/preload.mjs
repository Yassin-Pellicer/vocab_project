...
const chat = {
  chatSend: async (messages, accessToken) => {
    return await electron.ipcRenderer.invoke("chatSend", messages, accessToken);
  },
  chatConfig: async (language, accessToken) => {
    return await electron.ipcRenderer.invoke("chatConfig", language, accessToken);
  }
};
const auth = {
  deleteAccount: async (accessToken) => {
    return await electron.ipcRenderer.invoke("deleteAccount", accessToken);
  }
};
