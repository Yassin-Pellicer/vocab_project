...
async function syncFromDisk() {
  ...
  PreferencesContext.getState().loadConfig();

  await DictionaryContext.getState().loadConfig();
  await DictionaryContext.getState().loadAllTranslations();
}
...
async function syncNotesForActiveRoute() {
  ...
  NotesContext.getState().setTree(nextTree);
  NotesContext.getState().bumpReloadToken();
}
